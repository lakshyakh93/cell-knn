 ...
 */
void imageToPoint(Point<unsigned char, unsigned char> *point, unsigned char label,
		unsigned char *image, int length) {
	for (int i = 0; i < length; ++i) {
		point->getValues()[i] = static_cast<unsigned char> (image[i]);
	}
	point->setLabel(static_cast<unsigned char> (label));
}
