...
float distance(Point<unsigned char, unsigned char> &testPoint, Point<unsigned char, unsigned char> &trainPoint) {
	float result = 0.0;

	int size = testPoint.getDimension() / 16;
	vector unsigned char *trainVec = (vector unsigned char *) trainPoint.getValues();
	vector unsigned char *testVec = (vector unsigned char *) testPoint.getValues();
	vector unsigned char distVec;
	union {
	  int i[4];
	  vector signed int v;
	} sum;
	...
	for (int j = 0; j < size; j++) {
		...
		distVec = spu_absd(trainVec[j], testVec[j]);
		
		// Sum the 16 bytes of the vector
		sum.v = vec_sums((vector signed int)vec_sum4s(distVec, zero), (vector signed int)zero);
...
uint32_t calculate(Points<unsigned char, unsigned char> &test_points, Points<unsigned char, unsigned char> &training_points, SortedMap **maps) {
	for (int j = 0; j < test_points.getCount(); j++) {
		Point<unsigned char, unsigned char> *testPoint = test_points.getPoint(j);
...
uint32_t streamData(Points<unsigned char, unsigned char> &test_points) {
	...
	while (size_data > 0) {
		...
		Points<unsigned char, unsigned char> training_points0(cb.training_points_per_transfer, cb.training_dimension, trainingLabels[0], trainingValues[0]);
		...
		// TrainingPoints0 loaded, create Object
		Points<unsigned char, unsigned char> training_points1(cb.training_points_per_transfer, cb.training_dimension, trainingLabels[1], trainingValues[1]);
	...
	for (int i = 0; i < test_points.getCount(); i++) {
		Point<unsigned char, unsigned char> *temp = test_points.getPoint(i);
...
int main(unsigned long long speid, unsigned long long argp, unsigned long long envp) {
	...
	for (uint32_t test_point_transfer = 0, index = start; test_point_transfer < test_point_transfers_per_spu; test_point_transfer++, index
		...
		Points<unsigned char, unsigned char> *test_points = 0;
...
#ifdef DEBUG
			printf("Adresses = %d \t %d \n",((unsigned char *)testValues)[0], ((unsigned char *)testLabels)[0]); fflush(stdout);
#endif
			// TestPoints loaded, create Objects
			test_points = new Points<unsigned char, unsigned char>(cb.test_points_per_transfer, cb.test_dimension, testLabels, testValues);
		} else {
			test_points = new Points<unsigned char, unsigned char>(0, 0, testLabels, testValues);
			printf("%d SKIP\n", my_num);
			fflush(stdout);
		}

		#ifdef DEBUG
			printf("testpoints object created"); fflush(stdout);
#endif
